...
+(UIImage *)_ImageOfQRFromURL:(NSString *)urlStr codeSize:(CGFloat)codeSize logoName:(NSString *)logoName  radius: (CGFloat)radius borderWidth:(CGFloat)borderWidth borderColor:(UIColor *)borderColor{
    ...
    UIImage *orginQRImage =[self _ImageOfQRFromURL:urlStr codeSize:codeSize];
    ...
    UIImage *logoImage =[[UIImage imageNamed:logoName]_scaleToSize:waterImageRect.size];
    //生成水印图片 rect 从00 开始_scaleToSize:waterImageRect.size
    UIImage *waterImage =[UIImage _ClipImageRadiousWithImage:logoImage circleRect:CGRectMake(0, 0, waterImageRect.size.width, waterImageRect.size.height) radious:radius borderWith:borderWidth borderColor:borderColor];
    
  
    //添加水印图片
    
    finalImage =[UIImage _WaterImageWithImage:orginQRImage waterImage:waterImage waterImageRect:waterImageRect];
